...
exports.register = function(resp,conn,inp,res){
    var table = 'account';
    var main_qry = '';
...
exports.login = function(resp,conn,inp,res){
    var table = 'account';
    ...
    if(rows.length>0){
        ...
        if(first.username==user){
            conn.query(`SELECT a.*, b.active FROM riv_auth a INNER JOIN riv_account b ON a.id_account=b.id WHERE id_account LIKE "`+ first.id +`" LIMIT 1`,  function (error, rows, fields){
                var seconds = rows[0]
                ...
                if(rows.length>0){
                    ...
                    conn.query(`UPDATE riv_auth SET auth = "`+ auth +`", created_date = "` + datenow + `" WHERE id_account LIKE "`+ first.id +`" LIMIT 1;`);

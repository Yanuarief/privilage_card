...
module.exports = {
	...
	methods: {
        ...
        jumlah2an(){
            ...
            var point_bank = 0;   
            var sum_point = 0;               
            $(`#table_earn tbody > tr`).each(function(){
	...
	mounted(){
		...
		$('#form-input-earnpoint').createForm(act,
		[   
        ...
        $("#table_earn").ready(function (){
            $("#namaTenant_1").html(`
                <select class="form-control js-example-basic-single" style="width: 100%" id="namaTenant_select_1" name="namaTenant[]"}></select>
            ...
            $("#nominal_1").html(`<input class="form-control" name="nominal[]" autocomplete="off" id="nominal_1-currency"/>`)
            ...
            $("#namaBank_1").html(`
                <select class="form-control js-example-basic-single" style="width: 100%" id="namaBank_select_1" name="namaBank[]"}></select>
            ...
            $("#noKartu_1").html(`
                <input type="number" name="noKartu[]" id="noKartu" class="form-control">`
            )
            ...
            $("#table_earn").on("focusout","#noKartu_1 > #noKartu",  async function(){
                
                var getIdBank = $('#namaBank_1').find(':selected').val();
                var getTextBank = $('#namaBank_1').find(':selected').text();

                if(getTextBank == ""){
                    alert("Pilih Nama Bank Terlebih Dahulu")
                }else{
                    var value = this.value == '' ? 'noValid' : this.value
                    const getApi = await axios.get(rest["byidfull"] + `?id=${getIdBank}&produk=${getTextBank}&bin=${value}`);

                    var datas = getApi.data.data;

                    if(datas.length == 0){
                        $("#valid_1").text('No Valid')
                    }else{
                        $("#valid_1").text('Valid')
                    }
            ...
            $("#table_earn").on("keyup","input[name='nominal[]']",  async function(){
                vue.jumlah2an();
                var radioValue = $("input[name='bayar[1]']:checked").val();
                if(radioValue == "cash"){
                    $("#namaBank_1").hide()
                    $("#noKartu_1").hide()
                }else{
        ...
        $("#actionPlus").on("click", ".plus", function(e){
                ...
                <tr id="body_${getlastidrow}" data-idrow="${getlastidrow}">
                    <td>
                        <div id="namaTenant_${getlastidrow}">
                            <select class="form-control js-example-basic-single" style="width: 100%" id="namaTenant_select_${getlastidrow}" name="namaTenant[]"}></select>
                    ...
                    <td>
                        <div id="nominal_${getlastidrow}">
                            <input class="form-control" name="nominal[]" id="nominal_${getlastidrow}-currency" autocomplete="off" inputmode="numeric">
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div id="namaBank_${getlastidrow}">
                            <select class="form-control js-example-basic-single" style="width: 100%" id="namaBank_select_${getlastidrow}" name="namaBank[]"}></select>
            ...
            $(`#table_earn > tbody > tr #bayar_${getlastidrow}`).on("click" , "input[type='radio']", async function(){
                vue.jumlah2an();
                var radioValue = $(`#table_earn > tbody > tr #bayar_${getlastidrow}`).find(`input[name='bayar[${getlastidrow}]']:checked`).val();
                if(radioValue == "cash"){
                    $("#table_earn").find(`#namaBank_${getlastidrow}`).hide()
                    $("#table_earn").find(`#noKartu_${getlastidrow}`).hide()
                }else{
                    $("#table_earn").find(`#namaBank_${getlastidrow}`).show()
                    $("#table_earn").find(`#noKartu_${getlastidrow}`).show()
                }
            });
        })
